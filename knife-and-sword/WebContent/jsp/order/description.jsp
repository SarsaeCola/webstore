...
<link rel="stylesheet" type="text/css" href="<c:url value='/css/order/description.css'/>">
<title>镖局</title>
</head>
<body>
	<div class="divOrder">
...
<c:forEach items="${order.orderItemList }" var="item">
						<tr style="padding-top: 20px; padding-bottom: 20px;">
							<td class="td" width="400px">
								<div class="bookname">
								  <img align="middle" width="70" src="<c:url value='/${item.weapon.simg }'/>"/>
								  <a href="<c:url value='/BookServlet?method=load&bid=${item.weapon.wid }'/>">${item.weapon.wname }</a>
								</div>
							</td>
							<td class="td" >
								<span>&yen;${item.weapon.currentPrice }</span>
							</td>
							<td class="td">
...
<c:if test="${order.status eq 1 }">
	<a href="<c:url value='/OrderServlet?method=paymentPre&oid=${order.oid }'/>" class="pay">立即付款</a><br/>
</c:if>
