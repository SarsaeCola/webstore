...
public class AdminWeaponAddServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		sfu.setFileSizeMax(1024* 1024);//设置单个上传的文件上限为1MB
		/*
		...
		/*
		 * 4. 把List<FileItem>封装到Weapon对象中
		 * 4.1 首先把“普通表单字段”放到一个Map中，再把Map转换成Weapon和Category对象，再建立两者的关系
		 */
		...
		Weapon weapon = CommonUtils.toBean(map, Weapon.class);//把Map中大部分数据封装到Weapon对象中
		 ...
		 * 1. 获取真实路径
		 */
		String savepath = this.getServletContext().getRealPath("/weapon_img");
		/*
		 * 2. 创建目标文件
		 */
		...
		// 把图片的路径设置给weapon对象
		weapon.setBimg("weapon_img/" + filename);
		 ...
		 * 1. 获取真实路径
		 */
		savepath = this.getServletContext().getRealPath("/weapon_img");
		/*
		 * 2. 创建目标文件
		 */
		...
		// 把图片的路径设置给weapon对象
		weapon.setSimg("weapon_img/" + filename);
		
		
		
		
		// 调用service完成保存
		weapon.setWid(CommonUtils.uuid());
		WeaponService weaponService = new WeaponService();
		weaponService.add(weapon);
		
		// 保存成功信息转发到msg.jsp
		request.setAttribute("msg", "添加武器成功！");
	...
	private void error(String msg, HttpServletRequest request, HttpServletResponse response)
		...
		request.setAttribute("parents", new CategoryService().searchParents());//所有一级分类
		request.getRequestDispatcher("/adminJsps/cotent/weapon/add.jsp").
